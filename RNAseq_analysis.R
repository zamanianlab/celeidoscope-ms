...
cengen_df <- cengen_df %>%
  mutate(max_score = pmax(mKO2_Score, mCherry_Score, YFP_Score, GFP_Score),
    FP = case_when(mKO2_Score == max_score & mKO2_Score > score_cutoff ~ "mKO2",
      ...
      GFP_Score == max_score & GFP_Score > score_cutoff ~ "GFP", TRUE ~ NA_character_))

# UMAP FP plot
UMAP_fp_plot <- ggplot(cengen_df, aes(UMAP_1.y, UMAP_2.y)) +
  geom_point(color = "grey95", size = 0.2) +
  geom_point(data = filter(cengen_df, !is.na(FP)),
             aes(color = FP), size = 0.6, alpha = 0.4) +
  scale_color_manual(values = c("mKO2" = "orange", "mCherry" = "red",
               "YFP" = "yellow", "GFP" = "green")) +
...
cengen_df$Cell_Type <- case_when(cengen_df$Cell.type == "Intestine" ~ "Intestine",
  ...
  !(cengen_df$Cell.type %in% non_neuronal_types) ~ "Neuron", TRUE ~ NA_character_)

# UMAP cell type plot
UMAP_ct_plot <- ggplot(cengen_df, aes(UMAP_1.y, UMAP_2.y)) +
  geom_point(color = "grey95", size = 0.2) +
  geom_point(data = subset(cengen_df, !is.na(Cell_Type)),
             aes(color = Cell_Type), size = 0.6, alpha = 0.4) +
  theme_classic() +
  scale_color_manual(values = c("Neuron" = "orange", "Muscle/Mesoderm" = "red",
               "Intestine" = "yellow", "Pharyngeal_muscle" = "green"),
    breaks = c("Neuron", "Muscle/Mesoderm", "Intestine", "Pharyngeal_muscle"),
    labels = c("Neuron", "Muscle/Mesoderm", "Intestine","Pharyngeal muscle")) +
